...
Meaningful XP and Levels provides immediate, cumulative recognition for verified learning progress, preparation, completion, exploration, collaborative contribution, and post-course revision.
...
- distinguish learning, optional, examination, collaborative, and refresher XP where useful.
...
- recognition of verified Collaborative Challenge contribution;
- recognition of meaningful Post-course Knowledge Refresher completion;
...
- verified Collaborative Challenge contributions;
- structured Study Group activity only where it produces a trusted learning event;
- collaborative, optional, or competitive challenge results;
- Post-course Knowledge Refresher completion or maintained-knowledge results;
...
- source event or verified activity result;
- related course, course instance, group, challenge, or refresher when applicable;
...
Repeatable rules must state what counts as a new valid occurrence. For example, separate scheduled refresher occurrences may be distinct, while reprocessing the same challenge contribution or refresher attempt is not.

Configurable caps or diminishing returns may be used for repeatable challenge and refresher sources so that XP recognizes useful activity without encouraging farming.
...
If XP was awarded from incorrect data, fraud, an invalidated challenge or refresher result, or an incorrect manual decision, an authorized correction should create a reversing transaction. The original transaction remains in the audit history.
...
+20 XP — Completed an eligible Collaborative Challenge contribution
+30 XP — Completed an optional practice challenge
+25 XP — Completed a scheduled knowledge refresher
```
...
- **Socializers — medium:** verified contribution to a shared learning goal can earn recognition.
...
- Invalidating a rewarded challenge contribution or refresher result may reverse its XP according to the applicable rule.
...
### AC5 — Verified Collaborative Challenge contribution

Given a learner satisfies the configured minimum contribution for a Collaborative Challenge
and the applicable XP rule is enabled,
when the trusted contribution event is processed,
then the learner receives the configured XP once.
...
- Study Group activity only where it produces a trusted learning event;
- Collaborative, optional, and competitive challenge results;
- Post-course Knowledge Refresher eligibility, attempts, and results;
...
- proportion of XP coming from learning, optional, examination, collaborative, and refresher sources;
